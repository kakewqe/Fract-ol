...
/*   Updated: 2023/09/27 23:18:37 by zachamou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->re = (data->i - WIN_SIZE / 2.0) * data->zoom / WIN_SIZE + data->x;
	data->im = (data->j - WIN_SIZE / 2.0) * data->zoom / WIN_SIZE + data->y;
...
void	julia(t_data *data)
{
	...
	while (data->j < WIN_SIZE)
	{
		...
		{
			data->n = 0;
			my_putpixel(data, data->i, data->j, iterate_julia(data) * 0x000f0f
				/ 100);
			data->i++;
		}
		data->j++;
